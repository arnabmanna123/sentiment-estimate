...
class azure_operations:
    def __init__(self, connection_string, container_name):
        ...
        try:
            ...
            logging.exception(f"Failed to initialize Azure Blob Storage connection: {e}")
            raise
    ...
    def fetch_file_from_azure(self, file_path):
        ...
        """
        try:
            logging.info(f"Fetching file '{file_path}' from container '{self.container_name}'...")
            ...
            logging.info(f"Successfully fetched and loaded '{file_path}' from Azure that has {len(df)} records.")
            return df
            
        except AzureError as ae:
            logging.exception(f"❌ Azure-specific error while fetching '{file_path}': {ae}")
            return None
        except Exception as e:
            logging.exception(f"❌ Failed to fetch '{file_path}' from Azure: {e}")
            return None
    ...
    def upload_file_to_azure(self, file_path, local_file_path):
        ...
        """
        try:
            logging.info(f"Uploading file to '{file_path}' in container '{self.container_name}'...")
            blob_client = self.container_client.get_blob_client(file_path)
            ...
            logging.exception(f"❌ Azure-specific error while uploading to '{file_path}': {ae}")
            return False
        except Exception as e:
